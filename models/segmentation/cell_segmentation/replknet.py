...
class RepLKBlock(nn.Module):
    ...
    def __init__(self, in_channels, dw_channels, block_lk_size, small_kernel, drop_path, device="cuda", small_kernel_merged=False):
        super().__init__()
        self.pw1 = conv_bn_relu(in_channels, dw_channels, 1, 1, 0, groups=1).to(device)
        self.pw2 = conv_bn(dw_channels, in_channels, 1, 1, 0, groups=1).to(device)
        self.large_kernel = ReparamLargeKernelConv(in_channels=dw_channels, out_channels=dw_channels, kernel_size=block_lk_size,
                                                  stride=1, groups=dw_channels, small_kernel=small_kernel, small_kernel_merged=small_kernel_merged).to(device)
        self.lk_nonlinear = nn.ReLU().to(device)
        self.prelkb_bn = get_bn(in_channels).to(device)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
